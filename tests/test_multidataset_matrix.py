...
class MultidatasetMatrixTests(unittest.TestCase):
    ...
    def test_launcher_uses_immutable_snapshot_and_hierarchical_output(self):
        ...
        self.assertIn('MATRIX_WORKERS=${MATRIX_WORKERS:-2}', submitter)
        self.assertIn("MATRIX_WORKERS > 2", submitter)
        ...
        self.assertIn("--exclude=/experiments", submitter)
        self.assertNotIn("        --exclude=data \\", submitter)
        self.assertNotIn("        --exclude=experiments \\", submitter)
        self.assertIn("--exclude='data/**'", submitter)
